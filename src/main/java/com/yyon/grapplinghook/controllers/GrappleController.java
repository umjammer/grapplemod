...
public class GrappleController {
	...
	public Set<GrapplehookEntity> grapplehookEntities = new HashSet<>();
	public Set<Integer> grapplehookEntityIds = new HashSet<>();
	...
	public GrappleController(int grapplehookEntityId, int entityId, World world, Vec pos, int controllerid, GrappleCustomization custom) {
		...
		if (custom != null && custom.rocket) {
			clientProxy.updateRocketRegen(custom.rocket_active_time, custom.rocket_refuel_ratio);
		}
	}

	public void unattach() {
		if (clientProxy.unregisterController(this.entityId) != null) {
			this.attached = false;

			if (this.controllerId != GrapplemodUtils.AIRID) {
				new GrappleEndMessage(this.entityId, this.grapplehookEntityIds).send();
				clientProxy.createControl(GrapplemodUtils.AIRID, -1, this.entityId, this.entity.world, new Vec(0,0,0), null, this.custom);
			}
		}
	}
	...
	public void receivePlayerMovementMessage(float strafe,
		...
		playerMovement = playerMovementUnrotated.rotateYaw((float) (this.entity.getRotationVector().y * (Math.PI / 180.0)));
	}
		...
		if (this.attached) {
			if (entity != null) {
				if (true) {
					...
					playerpos = playerpos.add(new Vec(0, entity.getEyeHeight(entity.getPose()), 0));
					...
					if (this.custom.motor) {
						if (clientProxy.isKeyDown(GrappleKeys.key_motoronoff) && this.custom.motorwhencrouching) {
							motor = true;
						} else if (!clientProxy.isKeyDown(GrappleKeys.key_motoronoff) && this.custom.motorwhennotcrouching) {
							motor = true;
						}
					}
					...
					for (GrapplehookEntity hookEntity : this.grapplehookEntities) {
						...
						if (entity instanceof PlayerEntity player) {
							boolean isjumping = clientProxy.isKeyDown(GrappleKeys.key_jumpanddetach);
							isjumping = isjumping && !playerJump; // only jump once when key is first pressed
							playerJump = clientProxy.isKeyDown(GrappleKeys.key_jumpanddetach);
							if (isjumping) {
								// jumping
								if (onGroundTimer > 0) { // on ground: jump normally

								} else {
									double timer = clientProxy.getTimeSinceLastRopeJump(this.entity.world);
							...
							if (clientProxy.isKeyDown(GrappleKeys.key_slow)) {
								// slow down
							...
							if ((clientProxy.isKeyDown(GrappleKeys.key_climb) ||
									clientProxy.isKeyDown(GrappleKeys.key_climbup) ||
									clientProxy.isKeyDown(GrappleKeys.key_climbdown)) && !motor) {
								isClimbing = true;
								if (anchor.y > playerpos.y) {
									...
									if (clientProxy.isKeyDown(GrappleKeys.key_climb)) {
										...
										if (clientProxy.isMovingSlowly(this.entity)) {
											climbup = climbup / 0.3D;
										}
										if (climbup > 1) {climbup = 1;} else if (climbup < -1) {climbup = -1;}
									}
									else if (clientProxy.isKeyDown(GrappleKeys.key_climbup)) { climbup = 1.0; }
									else if (clientProxy.isKeyDown(GrappleKeys.key_climbdown)) { climbup = -1.0; }
									if (climbup != 0) {
		        	...
		        	Vec facing = new Vec(entity.getEyePos()).normalize();
...
//									vec pullxy = new vec(pull.x, 0, pull.z);
					...
					if (doJump) {
						...
						clientProxy.resetRopeJumpTime(this.entity.world);
						return;
					}
	...
	public void normalGround(boolean sliding) {
		...
		if (entity.isOnGround() || onGroundTimer > 0) {
			if (!sliding) {
				this.motion = Vec.motionVec(entity);
				if (clientProxy.isKeyDown(McKeys.keyBindJump)) {
					this.motion.y += 0.05;
				}
			}
		}
	...
	public void addHookEntity(GrapplehookEntity hookEntity) {
		...
		hookEntity.r = ((GrapplehookEntity) hookEntity).segmentHandler.getDist(Vec.positionVec(hookEntity), Vec.positionVec(entity).add(new Vec(0, entity.getEyeHeight(entity.getPose()), 0)));
...
//    	Block b = blockstate.getBlock();
	...
	public Vec rocket(Entity entity) {
		if (clientProxy.isKeyDown(GrappleKeys.key_rocket)) {
			rocket_on = clientProxy.getRocketFunctioning();
			double rocket_force = this.custom.rocket_force * 0.225 * rocket_on;
        	double yaw = entity.getRotationVector().y;
        	double pitch = -entity.getRotationVector().x;
	...
	public Vec getNearbyWall(Vec tryfirst, Vec trysecond, double extra) {
		float entitywidth = (float) this.entity.getBoundingBox().getYLength(); // TODO

		for (Vec direction : new Vec[] {tryfirst, trysecond, tryfirst.mult(-1), trysecond.mult(-1)}) {
			BlockHitResult raytraceresult = GrapplemodUtils.rayTraceBlocks(this.entity.world, Vec.positionVec(this.entity), Vec.positionVec(this.entity).add(direction.changeLen(entitywidth/2 + extra)), entity);
	...
	public boolean wallNearby(double dist) {
		...
		for (int i = 0; i < 4; i++) {
			...
			BlockHitResult raytraceresult = GrapplemodUtils.rayTraceBlocks(this.entity.world, Vec.positionVec(this.entity).add(corner1), Vec.positionVec(this.entity).add(corner2), entity);
			if (raytraceresult != null) {
				return true;
			}
		}
	...
	public boolean isWallRunning() {
		...
		if (isOnWall) {
			clientProxy.setWallrunTicks(clientProxy.getWallrunTicks()+1);
		}

		if (clientProxy.getWallrunTicks() < GrappleConfig.getConf().enchantments.wallrun.max_wallrun_time * 40) {
			if (!(playerSneak)) {
				// continue wallrun
				if (isOnWall && !this.entity.isOnGround() && this.entity.horizontalCollision) {
					return !(entity instanceof LivingEntity) || !((LivingEntity) entity).isClimbing();
				}

				// start wallrun
				if (clientProxy.isWallRunning(this.entity, this.motion)) {
					isOnWall = true;
					return true;
				}
			}
		...
		if (clientProxy.getWallrunTicks() > 0 && (this.entity.isOnGround() || (!this.entity.horizontalCollision && !wallNearby(0.2)))) {
	...
	public boolean applyWallrun() {
		...
		if (wallrun && !clientProxy.isKeyDown(GrappleKeys.key_jumpanddetach)) {
		...
		// jump
		boolean isjumping = clientProxy.isKeyDown(GrappleKeys.key_jumpanddetach) && isOnWall;
		isjumping = isjumping && !playerJump; // only jump once when key is first pressed
		playerJump = clientProxy.isKeyDown(GrappleKeys.key_jumpanddetach) && isOnWall;
		if (isjumping && wallrun) {
			clientProxy.setWallrunTicks(0);
			...
			clientProxy.playWallrunJumpSound(entity);
		}
