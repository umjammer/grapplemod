...
LOGGER.info("GrapplehookEntity::<init>: client: " + world.isClient() + ", " + hashCode());
	}
...
LOGGER.info("GrapplehookEntity::<init2>: client: " + world.isClient() + ", " + hashCode());
	}
...
//				    					}
	...
	public Packet<?> createSpawnPacket() {
		EntitySpawnS2CPacket packet = new EntitySpawnS2CPacket(this);
		PacketByteBuf buf = new PacketByteBuf(Unpooled.buffer());
		packet.write(buf);
		buf.writeInt(this.shootingEntity != null ? this.shootingEntity.getId() : 0);
		buf.writeBoolean(this.rightHand);
		buf.writeBoolean(this.isDouble);
		if (this.customization == null) {
			LOGGER.warn("GrappkehookEntity::createSpawnPacket: error: customization null");
		}
		buf.writeNbt(customization.writeNBT());
		return packet;
	}

	@Override
	public void onSpawnPacket(EntitySpawnS2CPacket packet) {
LOGGER.info("GrappkehookEntity::onSpawnPacket: " + packet.getId());
...
LOGGER.info("GrappkehookEntity::onRemoved");
new Exception(String.valueOf(getId())).printStackTrace();
	}
}
